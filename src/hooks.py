...
def _is_norm(m: nn.Module) -> bool:
    return isinstance(m, nn.LayerNorm) or "norm" in type(m).__name__.lower()
...
class ModelAdapter:
    ...
    def from_model(cls, model: nn.Module) -> "ModelAdapter":
        ...
        layers_path, layers = max(((n, m) for n, m in model.named_modules() if isinstance(m, nn.ModuleList)), key=lambda x: len(x[1]))

        parent_path = layers_path.rsplit(".", 1)[0] if "." in layers_path else ""
        parent = model.get_submodule(parent_path) if parent_path else model
        norm_children = [(f"{parent_path}.{n}" if parent_path else n) for n, m in parent.named_children() if _is_norm(m)]

        block_norms = [n for n, m in layers[0].named_children() if _is_norm(m)]
        return cls(layers_path=layers_path, final_norm_path=norm_children[-1], lm_head_path=lm_head_path, post_attn_norm_attr=block_norms[1])
...
class LayerHooks:
    ...
    def install(self, layer):
        ...
        self._handles.append(post_attn_norm.register_forward_pre_hook(self._capture_resid_attn))
        self._handles.append(layer.register_forward_hook(self._capture_resid_mlp_and_store))
