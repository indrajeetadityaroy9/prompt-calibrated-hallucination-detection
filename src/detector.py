...
from src.config import LayerHiddenStates, SIGNAL_NAMES, TokenSignals, DetectionResult
...
class Detector:
    ...
    def __init__(self, model: nn.Module, tokenizer) -> None:
        ...
        self.prompt_signals: np.ndarray | None = None
    ...
    def _prefill(self, input_ids: Tensor, prompt_len: int) -> tuple:
        ...
        def _make_prompt_pre_hook(lidx, end, tmp):
            def fn(module, args):
                tmp[lidx] = args[0][0, :end, :].detach()
            return fn

        def _make_prompt_hook(lidx, end, buf, tmp):
            def fn(module, args, output):
                buf[lidx] = {"h_resid_attn": tmp.pop(lidx), "h_resid_mlp": output[0][0, :end, :].detach()}
            return fn

        prompt_handles = []
        for layer_idx in range(self.num_layers):
            post_attn_norm = self.adapter.get_post_attn_norm(self._layers[layer_idx])
            prompt_handles.append(post_attn_norm.register_forward_pre_hook(_make_prompt_pre_hook(layer_idx, prompt_len, _prompt_attn_tmp)))
            prompt_handles.append(self._layers[layer_idx].register_forward_hook(_make_prompt_hook(layer_idx, prompt_len, prompt_per_layer, _prompt_attn_tmp)))
        ...
        self.prompt_stats, self.prompt_signals = self_calibrate(spectral_analyzer=self.spectral_analyzer, lm_head=self.lm_head, final_norm=self.final_norm, prompt_per_layer=prompt_per_layer, prefill_attentions=outputs.attentions)

        return outputs.past_key_values, outputs.logits[:, -1, :], outputs.attentions

    def compute_token_signals(self, logits: Tensor, attentions: tuple, seq_len: int) -> TokenSignals:
        ...
        fi = diffs.norm(dim=-1) ** 2 / H_token[:-1].norm(dim=-1) ** 2
    ...
    def _aggregate_results(self, token_results: list[TokenSignals], all_ids: Tensor, prompt_len: int) -> DetectionResult:
        ...
        signal_matrix = np.column_stack([[getattr(s, f) for s in token_results] for f in SIGNAL_NAMES])
    ...
    def _generation_loop(self, input_ids: Tensor, max_new_tokens: int, response_ids: list[int] | None = None) -> DetectionResult:
        ...
        token_results.append(self.compute_token_signals(next_logits.squeeze(0), prefill_attentions, prompt_len))
        ...
        for step in range(1, n_steps):
            ...
            token_results.append(self.compute_token_signals(logits.squeeze(0), outputs.attentions, seq_len))
