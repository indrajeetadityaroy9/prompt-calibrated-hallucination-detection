...
AG-SAR (Aggregated Signal Architecture for Risk) is a zero-shot, training-free hallucination detection system for autoregressive LLMs. It intercepts transformer internals during generation to extract five mechanistically-grounded signals and fuses them via cross-signal precision-weighted entropy-gated aggregation into per-token risk scores — no training data, calibration, or external models required. Works with any autoregressive generation (no retrieved context required).
...
1. **Prefill** (once per input): `ModelAdapter` auto-detects architecture via `nn.Module.get_submodule()` (LLaMA/Mistral/Qwen/Gemma, Phi, GPT-2/Neo, Falcon, GPT-NeoX). Prompt hidden states captured at midpoint layer. Prompt subspace (SVD + effective rank), prompt center, magnitude tau (PSP gate), SPT window size (effective rank of prompt singular values), prompt statistics (PIT reference values) from tail window sqrt(prompt_len). Hidden states in bfloat16. Attention backend set at model load time (`attn_implementation` in `from_pretrained()`).
...
- `detect(prompt, max_new_tokens)`: Generate text with hallucination detection from a prompt string.
- `detect_from_tokens(input_ids, max_new_tokens)`: Detection from pre-tokenized input.
- `score(prompt, response_text)`: Score pre-existing text via teacher-forced decode.
...
- `TokenSignals`: ENT/MLP/PSP/SPT/spectral_gap per token
...
- `run_eval.py`: Evaluation orchestration with YAML config support. Constructs prompts from question+context in eval harness.
...
- **Context-free**: All signals derive from prompt conditioning and generation dynamics — no retrieved context required.
