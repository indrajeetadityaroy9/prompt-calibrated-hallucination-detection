...
AG-SAR detects hallucinations by intercepting the model's internal computation during generation, extracting five mechanistically-grounded signals, and fusing them into a per-token risk score --- all without training, calibration data, or external models.

## Core Research Contributions

### 1. Causal Signal Decomposition

AG-SAR decomposes hallucination risk along the transformer's causal processing chain into five independent signals, each targeting a distinct failure mode:
...
- **Adaptive tail windowing**: The calibration window scales as sqrt(prompt_length), clamped to [16, prompt_len//2], balancing estimation accuracy against locality.
- **Signal-specific normalization**: CUS uses direct probability mapping (its [0,1] range has semantic meaning); POS, DPS, DoLa, and CGD use PIT normalization (their raw scales are input-dependent). DoLa polarity is inverted (`higher_is_riskier=False`) since high layer-contrast indicates factual content.
...
- **Registry-derived fallbacks**: When calibration fails, bounded signals fall back to direct mode, unbounded signals use uniform MAD sigma.
...
├── __init__.py                    # Public API: Detector, Config, DetectionResult, RiskySpan
├── config.py                      # Config, TokenSignals, SIGNAL_REGISTRY, DetectionResult
...
├── hooks/
│   ├── adapter.py                 # ModelAdapter: architecture auto-detection
...
├── input_builder.py               # Input tokenization (Context/Question/Answer format)
...
```python
from ag_sar import Detector, Config
from transformers import AutoModelForCausalLM, AutoTokenizer

model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.1-8B-Instruct")
tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct")

detector = Detector(model, tokenizer, Config())
...
|--------|-------|---------|----------------|------------|
| **AG-SAR** | — | 5 (CUS, POS, DPS, DoLa, CGD) | Yes | Yes |
