...
def calibrate_cusum(signal_matrix: np.ndarray) -> CalibrationStats:
    ...
    return CalibrationStats(mu=mu, precision=precision, tau=tau, h=float(np.max(cusum)))
...
def compute_cusum_risks(signal_matrix: np.ndarray, stats: CalibrationStats):
    ...
    cusum = np.zeros(n)
    for i in range(n):
        cusum[i] = max(0.0, (cusum[i - 1] if i > 0 else 0.0) + dists[i] - stats.tau)
